...
export default function App() {
  ...
  const [pinInput, setPinInput] = useState(''); // Para sa login input control

  // BASE URL LAMANG PARA SA TAMANG ROUTING NG VERCEL
  const BACKEND_URL = 'https://admin-office-backend.vercel.app';

  // Helper para makuha ang token na naka-save sa browser
  const getAuthHeader = () => {
  ...
  const fetchTransactions = async () => {
    try {
      ...
      if (result.success) {
        ...
        // Kung hindi authorized ang token, ibalik sa login
  ...
  // LOGIN FUNCTION: Tumatawag na ngayon sa Backend API!
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (result.success) {
        // I-save ang PIN pansamantala bilang token para sa checkAuth middleware ng backend
        localStorage.setItem('admin_token', pinInput);
        setPinInput('');
        setView('dashboard');
      } else {
        alert(result.message || 'Maling Password!');
      }
    } catch (error) {
      console.error("Login Error:", error);
      alert('❌ Hindi makakonekta sa Backend Server!');
    }
  };
  ...
  return (
    <div style={{ fontFamily: 'sans-serif', backgroundColor: '#f3f4f6', minHeight: '100vh', padding: '20px' }}>
      <div style={{ display: 'flex', justifyContent: 'center', gap: '20px', marginBottom: '30px' }}>
        ...
        <button onClick={() => setView(localStorage.getItem('admin_token') ? 'dashboard' : 'login')} style={{ padding: '10px 20px', cursor: 'pointer', backgroundColor: view === 'dashboard' || view === 'login' ? '#16a34a' : '#fff', color: view === 'dashboard' || view === 'login' ? '#fff' : '#333', border: '1px solid #ccc', borderRadius: '5px', fontWeight: 'bold' }}>📊 Admin Dashboard</button>
      </div>
      ...
      {view === 'login' && (
        <div style={{ backgroundColor: 'white', padding: '25px', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)', maxWidth: '350px', margin: '40px auto', textAlign: 'center' }}>
          ...
          <form onSubmit={handleLogin} style={{ display: 'flex', flexDirection: 'column', gap: '12px', marginTop: '15px' }}>
            <input type="password" name="pwd" placeholder="Enter PIN" value={pinInput} onChange={(e) => setPinInput(e.target.value)} required style={{ padding: '10px', borderRadius: '5px', border: '1px solid #ccc', textAlign: 'center' }}/>
      ...
      {view === 'dashboard' && (
        <div style={{ backgroundColor: 'white', padding: '20px', borderRadius: '10px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '15px' }}>
            ...
            <button onClick={handleLogout} style={{ padding: '8px 15px', backgroundColor: '#dc2626', color: 'white', border: 'none', borderRadius: '5px', fontWeight: 'bold', cursor: 'pointer' }}>🚪 Lock Dashboard</button>
          </div>
